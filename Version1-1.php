# Build WordPress plugin v1.1.0 with "Edit Product" tab (Classic editor via wp_editor),
# short_summary field, and inline action buttons. Includes all prior features.
import os, textwrap, zipfile

root = "/mnt/data/virtual-product-pages-v1.1.0"
...
class VPP_Plugin {
    ...
    public function enqueue_assets() {
        wp_register_style('vpp-styles', plugins_url('assets/vpp.css', __FILE__), [], '1.1.0');
    ...
    public function handle_edit_save() {
        ...
        if ($this->column_exists($mysqli, $table, 'last_tidb_update_at')) {
            $set_parts.append("last_tidb_update_at = NOW()");
        }
        ...
        // Handle post-actions
        if ($ok and isset($_POST['do_save_purge'])) {
        ...
        if ($ok and isset($_POST['do_push_algolia'])) {
            $p = $this->fetch_product_by_id($id, $tmp);
            if ($p and $this->push_algolia($p, $tmp2)) {
        ...
        if (isset($_POST['do_save_view']) and $ok) {
    ...
    private function db_connect(&$err = null) {
        ...
        if (defined('MYSQLI_OPT_SSL_VERIFY_SERVER_CERT')) {
            @mysqli_options($mysqli, MYSQLI_OPT_SSL_VERIFY_SERVER_CERT, $ssl_ca and @file_exists($ssl_ca));
        }
...
zip_path = "/mnt/data/virtual-product-pages-v1.1.0.zip"
